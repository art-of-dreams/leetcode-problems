...
// in the inclusive range [left, right] having a prime number
...
static int filled_bits_count(int number) {
    int count = 0;
...
int countPrimeSetBits(int left, int right) {
    ...
    while (left <= right) {
        if (is_prime(filled_bits_count(left))) {
            count += 1;
        }
        left += 1;
    }
